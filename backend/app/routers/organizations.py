...
async def list_sites(org_id: int, db: AsyncSession = Depends(get_db)):
...
async def create_site(org_id: int, body: SiteCreate, db: AsyncSession = Depends(get_db)):
...
async def delete_site(org_id: int, site_id: int, db: AsyncSession = Depends(get_db)):
